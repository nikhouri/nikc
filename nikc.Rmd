...
* Legend moved to top (categories visible first, then data)
* Fit lines using a linear model (default is `loess`)
...
ggplot(df) +
    ...
    geom_point(aes(x=cpu, y=temp, color=label, shape=label),
               alpha=0.75, size=3) +
    geom_smooth(data=df[df$label=="mail",], aes(x=cpu, y=temp), method=lm, 
                formula=y ~ x, color="#009988", size=1.5) +
    geom_smooth(data=df[df$label=="web",], aes(x=cpu, y=temp), method=lm, 
                formula=y ~ x, color="#EE7733", size=1.5) +
    # Axis scales and colour scales
    scale_color_discrete(type=tolmini) +
    # Plot labels
