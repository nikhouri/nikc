...
Here are some colourblind-safe palettes. The default R ones are not.
...
## Continuous Colours
 [viridis](https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html), is colourblind-safe and built in to ggplot2 now anyway. There are alternative viridis sets - but these need the library.
...
Histogram with a cumulative curve on the second axis. Secondary axes are not easy to do in ggplot2, this example uses a manual factor to adjust the scaling.
...
Boxplots showing the underlying data (jittered).
